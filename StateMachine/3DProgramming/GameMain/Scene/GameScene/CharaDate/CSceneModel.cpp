...
/*パックの初期位置*/
#define PUCK_INIT_POS -12.0f,0.0f,-25.0f
/*静的初期化*/
...
//	CTaskManager::GetInstance()->Add(pl);//タスクに追加
	CTaskManager::GetInstance()->Add(new CStatePlayer(model, PlayerPos));
...
void CSceneModel::Update() {
	if (CGameScene::Transition == CGameScene::E_ACTIVE){
		...
		mRotation.y += ANGLE_SPEED * CMouse::mDirection;
		
		CMouse::mDirection = 0.0f;
