...
void CQuest::Init(){
	...
	mQuestName00.mPosition = NAME_POS0;

	mShadow[0] = mQuestName00;
	mShadow[0].SetColor(SKELETON_BLACK_COLOR);
	mShadow[0].mPosition = NAME_POS0S;
	...
	mQuestName01.mPosition = NAME_POS1;

	mShadow[1] = mQuestName01;
	mShadow[1].SetColor(SKELETON_BLACK_COLOR);
	mShadow[1].mPosition = NAME_POS1S;
	...
	mQuestName02.mPosition = NAME_POS2;

	mShadow[2] = mQuestName02;
	mShadow[2].SetColor(SKELETON_BLACK_COLOR);
	mShadow[2].mPosition = NAME_POS2S;
	...
	mBackButton.mPosition = BACK_POS;
...
void CQuest::Update(){
	...
	mQuestName01.mPosition = NAME_POS1;
	mQuestName02.mPosition = NAME_POS2;
	//mQuestName03.mPosition = NAME_POS3;

	//クリック時に押し込まれたような形に
	if (CCollision2D::Collision2D(mCursor, mQuestName00)){mQuestName00.SetColor(GRAY_COLOR);
	if (CMouse::GetInstance()->mOneLeftFlag){ mQuestName00.mPosition = NAME_POS0S; mQuestName00.SetColor(DARK_GRAY_COLOR); }
	}
	if (CCollision2D::Collision2D(mCursor, mQuestName01)){mQuestName01.SetColor(GRAY_COLOR);
	if (CMouse::GetInstance()->mOneLeftFlag){ mQuestName01.mPosition = NAME_POS1S;  mQuestName01.SetColor(DARK_GRAY_COLOR); }
	}
	if (CCollision2D::Collision2D(mCursor, mQuestName02)){mQuestName02.SetColor(GRAY_COLOR);
	if (CMouse::GetInstance()->mOneLeftFlag){ mQuestName02.mPosition = NAME_POS2S; mQuestName02.SetColor(DARK_GRAY_COLOR); }
	}
...
mBackButton.SetColor(GRAY_COLOR);
	...
	mCursor.mPosition = CMouse::GetInstance()->mPos;
