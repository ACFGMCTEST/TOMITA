...
//カラーを設定
void CRectangle::SetDiffuse(float cr, float cg, float cb, float ca){
	r = cr;
	g = cg;
	b = cb;
	a = ca;
...
v0～v3：頂点１～４までの座標
*/
void CRectangle::SetVertex(const CVector3 &v0, const CVector3 &v1, const CVector3 &v2, const CVector3 &v3) {
	mEnabled = true;//設定された
	mVertex[0] = v0;
	mVertex[1] = v1;
	mVertex[2] = v2;
...
void CRectangle::Fade(float speed, float alaha){
...
/*フェード関数*/
void CRectangle::FadeOut(float speed, float alaha){
	float mFade4 = a - speed;		//フェード関数で使う
	mFade4 -= speed;
	if (mFade4 <= alaha){
		mFade4 = alaha;
...
/*テクスチャ横スクロール関数*/
void CRectangle::TexSideScroll(float speed){
...
/*テクスチャ縦スクロール関数*/
void CRectangle::TexVerticalScroll(float speed){
	float left, top, right, bottom;
	/*保存*/
	left = mSaveTexVer[E_LEFT];
	top = mSaveTexVer[E_TOP];
