...
void CBox::SetTex(E_TexBox eTexState){
	switch (eTexState)
	{
		/*坂*/
	case CBox::E_SLOPE:
		mpTextrue = CLoadTexManager::GetInstance()->mpSlope;
		AllUV(mpTextrue, TEX_SLOPE_SIZE);
		break;
		/*箱*/
	case CBox::E_BOX:
		mpTextrue = CLoadTexManager::GetInstance()->mpBox;
		AllUV(mpTextrue, TEX_BOX_SIZE);
		break;
			/*高速床*/
	case CBox::E_SPEED_UP:
		mpTextrue = CLoadTexManager::GetInstance()->mpFastFloor;
		AllUV(mpTextrue, TEX_SPEED_UP_SIZE);
		/*減速床*/
		break;
	case CBox::E_SPEED_DOWN:
		mpTextrue = CLoadTexManager::GetInstance()->mpSlowFloor;

		AllUV(mpTextrue, TEX_SPEED_DOWN_SIZE);
		break;
	case CBox::E_NET:
		mpTextrue = CLoadTexManager::GetInstance()->mpNet;

		AllUV(mpTextrue, TEX_NET_SIZE);
		break;
	case CBox::E_WALL:
		mpTextrue = CLoadTexManager::GetInstance()->mpWall;
		AllUV(mpTextrue, TEX_WALL_SIZE);
		break;
	}
}
